...
class NormalDistribution {
    ...
    normalCDF(x) { return 0.5 * (1 + this.erf(x / Math.SQRT2)) }
    ...
    normalCDFGeneral(x, mu, sigma)        { return this.normalCDF((x - mu) / sigma) }
    normalInverseCDFGeneral(x, mu, sigma)  { return mu + sigma * this.normalInverseCDF(x) }
    ...
    action() {
        ...
        ExtText.tex(`\\Phi(${val}) = ${this.normalCDF(val)}`, "p7Res1")
        try { ExtText.tex(`\\Phi^{-1}(${val}) = ${this.normalInverseCDF(val)}`, "p7Res2") } catch (_) {}
